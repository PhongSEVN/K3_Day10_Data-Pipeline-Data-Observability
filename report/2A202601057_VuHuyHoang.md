...
| ------------------ | --------------------- | ---------------- | ----------------- | -------------------------------------------- |
| Corruption & repair | `src/ingestion/corruption.py` | `data/clean/` (baseline) | Corruption scenarios (xóa record, blank summary, noise, truncate title, stale date, duplicate) + dữ liệu repair hợp lệ | [Hoàn thành/Một phần/Chưa hoàn thành] |
